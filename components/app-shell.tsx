...
import { BarChart3, Bot, LayoutDashboard, LogOut, Settings, UserCircle, BookOpen } from 'lucide-react'
...
const navItems = [
  ...
  { href: '/instructor', label: 'DSA Instructor', icon: BookOpen },
      ...
      <aside className="fixed left-0 top-0 hidden h-screen w-[240px] border-r border-border bg-white p-4 md:block">
        ...
        <div className="absolute bottom-4 left-4 right-4 border-t border-border pt-4">
          <div className="flex items-center justify-between gap-2">
            <Link
              href="/settings"
              className="flex items-center gap-2 text-sm text-primary hover:text-brand transition-colors font-medium truncate"
            >
              <UserCircle className="h-4 w-4 shrink-0" />
              <span className="truncate">{user.leetcodeUsername ?? user.firstName ?? 'Profile'}</span>
            </Link>
            <SignOutButton>
              <button
                className="text-secondary hover:text-danger p-1 rounded-md hover:bg-surface transition-colors shrink-0"
                aria-label="Sign out"
              >
      ...
      <nav className="fixed bottom-0 left-0 right-0 grid grid-cols-5 border-t border-border bg-white md:hidden">
        {[
          ...
          <Link
            ...
            href={item.href}
            className={cn(
              'flex flex-col items-center justify-center text-center gap-1 px-1 py-2 text-[10px] sm:text-[11px] text-secondary',
